package ObjectRepositoryUtility;

import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.PageFactory;

public class opportunityInformationPage {

	WebDriver driver;

	public opportunityInformationPage(WebDriver driver) {
		this.driver = driver;
		PageFactory.initElements(driver, this);
	}

	@FindBy(className = "dvHeaderText")
	private WebElement orgHeaderInfo;

	public String orgHeaderInfovalidations() {
		return orgHeaderInfo.getText();
	}

	@FindBy(className = "dvHeaderText")
	private WebElement OpportunityHeaderInfo;

	public String OppHeaderInfoValidation() {
		return OpportunityHeaderInfo.getText();
	}

	public WebElement getOrgHeaderInfo() {
		return orgHeaderInfo;
	}

	public WebElement getOpportunityHeaderInfo() {
		return OpportunityHeaderInfo;
	}
}
